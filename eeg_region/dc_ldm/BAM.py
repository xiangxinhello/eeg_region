...
class ChannelGate(nn.Module):
    ...
    def forward(self, in_tensor):
        ...
        avg_pool = F.avg_pool2d(in_tensor, (h, w), stride=(h, w))
        # C∗H∗W -> C*1*1 -> C*H*W
        # return self.gate_c(avg_pool).unsqueeze(2).unsqueeze(3).expand_as(in_tensor)
        res = self.gate_c(avg_pool)
        res2 = res.unsqueeze(2).unsqueeze(3)
        res3 = res2.expand_as(in_tensor)
        return res3
...
# data = np.load("D:\\4D-CRNN-master\\4D-CRNN-master\X89.npy")
# data = data[:2, :, :, 1:5]
# b, h, w, c = data.shape
# data = torch.tensor(data)
# data = data.permute(0, 3, 1, 2)
# model = BAM(gate_channel=c)
# res = model(data)