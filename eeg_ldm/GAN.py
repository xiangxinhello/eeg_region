...
# )
#
# train_loader = DataLoader(eeg_latents_dataset_train, batch_size=4, shuffle=False)
# test_loader = DataLoader(eeg_latents_dataset_test, batch_size=4, shuffle=False)
...
class ConvNet(nn.Module):
    def __init__(self, input_channels, output_size):
        ...
        self.conv1d = nn.Conv1d(input_channels, 512, kernel_size=3)  # 一维卷积层
        self.global_avg_pool = nn.AdaptiveAvgPool1d(1)  # 全局平均池化
        self.fc = nn.Linear(512, 65536)
...
class Self_Attn(nn.Module):
    ...
    def forward(self, x):
        ...
        proj_value = proj_value.reshape(100, 4)     #
        out1 = torch.matmul(proj_value, attention).view(4,100)  # 20x20 20x20 -> 20x20
        # out1 = F.relu(out1)
...
conv_liner = ConvNet(input_channels=128, output_size=512).to(device)
